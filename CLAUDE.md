...
**目标服务器**：HZUS（SSH Host: `HZ-US-code_deploy`）

**部署方式**：Docker 容器化部署（默认）
...
**部署命令**：
- 一键部署：在项目根目录执行 `./deploy.sh`
- 脚本自动完成：rsync 同步代码 → Docker 构建 → 停止旧容器 → 启动新容器 → 清理镜像

**注意事项**：
- 3000-3010 端口段已被其他服务占用，禁止使用
- 部署前确保本地代码能通过 TypeScript 编译（`pnpm build`）
- `.env.local` 等环境变量文件通过 rsync 同步，不提交到 Git